// =============================================================================
// Piezas de ajedrez — set clásico de torneo "Cburnett" (dominio público,
// el de Lichess/Wikipedia). SVG con color explícito, NO Unicode, así iOS Safari
// no las pinta como emoji. Las NEGRAS llevan un borde claro (drop-shadow) para
// que resalten en cuadros oscuros del tablero.
// =============================================================================
...
const WHITE_FILTER = 'drop-shadow(0 1px 1.4px rgba(0,0,0,.55))';
// Halo claro + sombra: hace visibles las negras sobre cuadros oscuros.
const BLACK_FILTER = 'drop-shadow(0 0 1px rgba(236,216,170,.95)) drop-shadow(0 0 1px rgba(236,216,170,.9)) drop-shadow(0 1px 1.5px rgba(0,0,0,.5))';

export function ChessPiece({ type, white, size = '92%' }: { type: string; white: boolean; size?: number | string }) {
  const key = (white ? 'w' : 'b') + type.toUpperCase();
  ...
  return (
    <svg
      ...
      width={size}
      height={size}
      style={{ display: 'block', filter: white ? WHITE_FILTER : BLACK_FILTER }}
      aria-hidden="true"
